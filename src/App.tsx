...
const copy = {
  ...
  en: {
    ...
    hero: "Luxury, redefined for you.",    sub: "Your style. Your signature.",
  ...
  return (
    <div className={"app " + (rtl ? "rtl" : "ltr")}>
      ...
      <header>
        ...
        <div className="tools">          <div className="language"><button>{t.lang}⌄</button><div className="languageMenu">{(["ar","fr","en"] as const).map(l=><button key={l} onClick={()=>{setLang(l);setOpen(false)}}>{l==="ar"?"العربية":l==="fr"?"Français":"English"}</button>)}</div></div>
      ...
      <main>
        ...
        <section id="shop" className="shop">
          <div className="shopControls"><label className="searchBox">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder={lang==="ar"?"ابحثي عن منتج…":lang==="fr"?"Rechercher un produit…":"Search products…"}/></label></div>
          <div className="shopHead"><div className="shopViewTools"><div className="categoryPills">{["all","fashion","beauty","fragrance","accessories"].map((k,i)=><button key={k} className={category===k?"active":""} onClick={()=>setCategory(k)}>{lang==="ar"?["الكل","ملابس","جمال","عطور","أكسسوارات"][i]:["ALL","MODE","BEAUTY","FRAGRANCE","ACCESSORIES"][i]}</button>)}</div><select className="sortBox" value={sort} onChange={e=>setSort(e.target.value)}><option value="newest">{lang==="ar"?"الأحدث":"NEWEST"}</option><option value="low">{lang==="ar"?"الأقل سعراً":"PRICE ↑"}</option><option value="high">{lang==="ar"?"الأعلى سعراً":"PRICE ↓"}</option></select><div className="viewToggle" aria-label="View mode"><button className={viewMode===1?"active":""} aria-label="Single column view" onClick={()=>setViewMode(1)}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 4v16"/></svg></button><button className={viewMode===2?"active":""} aria-label="Grid view" onClick={()=>setViewMode(2)}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/></svg></button></div></div></div>
          <div className={"products products-"+viewMode}>{filteredProducts.map(p=>{const i=images.products.indexOf(p),photoIndex=photoIndexes[i]??0;return <article className="product" key={p.name[2]} onClick={()=>{if(!swiping.current){setSelectedProduct(i);setModalPhoto(photoIndex)}}}><div className="pic productSwipe" onPointerDown={e=>{swipeStart.current={x:e.clientX,index:photoIndex};swiping.current=false;e.currentTarget.setPointerCapture?.(e.pointerId)}} onPointerUp={e=>{const dx=e.clientX-swipeStart.current.x;if(Math.abs(dx)>35){const next=dx<0?(swipeStart.current.index+1)%p.photos.length:(swipeStart.current.index-1+p.photos.length)%p.photos.length;setPhotoIndexes(s=>({...s,[i]:next}));swiping.current=true;window.setTimeout(()=>{swiping.current=false},100)}}} onPointerCancel={()=>{swiping.current=false}}>
            <img key={photoIndex} className="productImageSmooth" src={p.photos[photoIndex]} alt={p.name[labelIndex]} draggable="false"/><button className={"favorite " + (favorites.includes(i) ? "active" : "")} aria-label={favorites.includes(i) ? "Remove from favorites" : "Add to favorites"} onClick={e=>{e.stopPropagation();setFavorites(f=>f.includes(i)?f.filter(x=>x!==i):[...f,i])}}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20.25S4 15.7 4 9.9C4 6.8 6.1 5 8.7 5c1.47 0 2.7.69 3.3 1.88C12.6 5.69 13.83 5 15.3 5 17.9 5 20 6.8 20 9.9c0 5.8-8 10.35-8 10.35Z"/></svg></button><div className="swipeDots">{p.photos.map((_,pi)=><i key={pi} className={pi===photoIndex?"active":""}/>)}</div>
          </div><div className="pmeta"><div className="productName"><h3>{p.name[labelIndex]}</h3></div><div className="productBuyRow"><button className="quickAdd" aria-label={t.add} onClick={e=>{e.stopPropagation();addToCart(p.name[2])}}><span className="cartIcon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 4h2l2.2 10.2a2 2 0 0 0 2 1.6h7.9a2 2 0 0 0 1.9-1.4L21 7H6"/><circle cx="10" cy="19" r="1"/><circle cx="18" cy="19" r="1"/></svg></span></button><strong>{p.price[labelIndex]}</strong></div></div></article>})}</div>
        </section>
